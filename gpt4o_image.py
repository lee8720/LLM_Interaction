...
# Load the Excel file that contains case metadata
file_path = "path/to/your/Problem.xlsx"

# 1) Load JSON containing image download links
with open("path/to/your/case_download_links.json", "r", encoding="utf-8") as f:
    case_image_links = json.load(f)

# 2) Helper function to extract numeric index from filename
def extract_number(file_name):
    match = re.search(r'\d+', file_name)
    return int(match.group()) if match else float('inf')

# 3) Sort images within each case by numeric order
...
# Maximum number of retry attempts per case
max_retries   = 5
correct_count = 0
...
for idx in range(len(df)):
    ...
    while retries < max_retries and not success:
        try:
            ...
            if "ranking" not in response_data:
                raise ValueError("Invalid JSON: 'ranking' key missing.")
            ...
            top3_threshold = sorted(score_norm.values(), reverse=True)[2]

            for item in response_data["ranking"]:
                diag      = item["diagnosis"]
                score     = score_norm.get(diag, 0.0)
                rationale = item.get("rationale", "") if score >= top3_threshold else ""
            ...
            print(f"\nCase {case_no} processed successfully.")
            for item in response_data["ranking"]:
                diag      = item["diagnosis"]
                score     = score_norm.get(diag, 0.0)
            ...
            if top_pred.strip().lower() == ground_truth.strip().lower():
                correct_count += 1
                print(f"[Correct] Case {case_no}: {top_pred}")
            else:
                print(f"[Wrong]   Case {case_no}: Predicted '{top_pred}' | Answer '{ground_truth}'")

            print(f"Running accuracy: {correct_count}/{idx + 1} ({correct_count / (idx + 1):.2%})\n")

        except Exception as e:
            retries += 1
            print(f"Error processing case {case_no}: {str(e)}")
    ...
    # Save after every case to preserve progress in case of crash
